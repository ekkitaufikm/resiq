<!-- Container-fluid starts-->
<div class="container-fluid pt-4">
    <div class="row project-cards">
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header p-3">
                    <div class="row">
                        <div class="col-lg-6">
                            <h5 class="card-title">Stok Paving</h5>
                        </div>
                        <div class="col-lg-6">
                            <a href="<?= site_url('produksi/tambahstok_paving') ?>" class="btn btn-sm btn-primary float-end">Tambah Data</a>
                        </div>
                    </div>
                </div>

                <div class="card-body table-responsive">
                    <table class="display" id="basic-1">
                        <thead>
                            <tr>
                                <th width="5%"> </th>
                                <th>Nama</th>
                                <th>Jumlah Stok</th>
                                <th width="10%">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>1</td>
                                <td>Paving Block Rumput</td>
                                <td>1000</td>
                                <td>
                                    <div class="btn-group btn-group-square" role="group" aria-label="">
                                        <a href="https://resiq.id/lingkar/plastik-cara/edit/1" class="btn btn-primary" title="Ubah Data">Edit</a>
                                        <a href="javascript:void(0);" class="btn btn-danger" title="Hapus Data">Hapus</a>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td>2</td>                               
                                <td>Paving Block Bata</td>
                                <td>500</td>
                                <td>
                                    <div class="btn-group btn-group-square" role="group" aria-label="">
                                        <a href="https://resiq.id/lingkar/plastik-cara/edit/2" class="btn btn-primary" title="Ubah Data">Edit</a>
                                        <a href="javascript:void(0);" class="btn btn-danger" title="Hapus Data">Hapus</a>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td>3</td>                                
                                <td>Paving Block Cacing</td>
                                <td>1200</td>
                                <td>
                                    <div class="btn-group btn-group-square" role="group" aria-label="">
                                        <a href="https://resiq.id/lingkar/plastik-cara/edit/2" class="btn btn-primary" title="Ubah Data">Edit</a>
                                        <a href="javascript:void(0);" class="btn btn-danger" title="Hapus Data">Hapus</a>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td>4</td>                               
                                <td>Paving Block Segi Enam(Hexagon)</td>
                                <td>100</td>
                                <td>
                                    <div class="btn-group btn-group-square" role="group" aria-label="">
                                        <a href="https://resiq.id/lingkar/plastik-cara/edit/2" class="btn btn-primary" title="Ubah Data">Edit</a>
                                        <a href="javascript:void(0);" class="btn btn-danger" title="Hapus Data">Hapus</a>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td>5</td>        
                                <td>Paving Block Topi(Uskup)</td>
                                <td>200</td>
                                <td>
                                    <div class="btn-group btn-group-square" role="group" aria-label="">
                                        <a href="https://resiq.id/lingkar/plastik-cara/edit/2" class="btn btn-primary" title="Ubah Data">Edit</a>
                                        <a href="javascript:void(0);" class="btn btn-danger" title="Hapus Data">Hapus</a>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td>6</td>
                                <td>Paving Block Kanstin</td>
                                <td>900</td>
                                <td>
                                    <div class="btn-group btn-group-square" role="group" aria-label="">
                                        <a href="https://resiq.id/lingkar/plastik-cara/edit/2" class="btn btn-primary" title="Ubah Data">Edit</a>
                                        <a href="javascript:void(0);" class="btn btn-danger" title="Hapus Data">Hapus</a>
                                    </div>
                                </td>
                            </tr>

                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- Container-fluid Ends-->